...
- `server/routes.ts` - API endpoints for token economy, email management, admin, deployments
...
- `client/src/pages/dashboard.tsx` - Main Mini App UI (dashboard, inbox, earn tokens+gems, admin panel)
...
- `POST /api/reward-ad` - Reward tokens + gems after ad watch
...
- Watch Ad: +20 tokens + 0.2 gems (10-second timer with multi-ad)
...
- Social Bar + Native Banner: Displayed inside the ad modal (main document context)
- Direct Link + Popunder: Triggered when "Watch Ad" button is clicked
- cleanupAdScripts() removes all ad scripts when modal closes
...
- **Tools Tab**: Add/deduct tokens (toggle), mass gift, broadcast messages via Telegram
